...
#if defined IS_TEST_NET && defined DPN_ENABLE_SMT
...
BOOST_AUTO_TEST_CASE( smt_limit_order_create_authorities )
{
   try
   {
      ...
      op.expiration = db->head_block_time() + fc::seconds( DPN_MAX_TIME_UNTIL_EXPIRATION );

      tx.operations.push_back( op );
      tx.set_expiration( db->head_block_time() + DPN_MAX_TIME_UNTIL_EXPIRATION );

      BOOST_TEST_MESSAGE( "--- Test failure when no signature." );
      DPN_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_missing_active_auth );
      ...
      sign( tx, alice_private_key );
      DPN_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_duplicate_sig );

      BOOST_TEST_MESSAGE( "--- Test failure with additional incorrect signature" );
      tx.signatures.clear();
      sign( tx, alice_private_key );
      sign( tx, bob_private_key );
      DPN_REQUIRE_THROW( db->push_transaction( tx, database::skip_transaction_dupe_check ), tx_irrelevant_sig );
...
BOOST_AUTO_TEST_CASE( smt_limit_order_create_apply )
{
   try
   {
      ...
      tx.set_expiration( db->head_block_time() + DPN_MAX_TIME_UNTIL_EXPIRATION );
      sign( tx, bob_private_key );
      DPN_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::exception );

      BOOST_REQUIRE( limit_order_idx.find( boost::make_tuple( "bob", op.orderid ) ) == limit_order_idx.end() );
      BOOST_REQUIRE( db->get_balance( bob_account, DPN_SYMBOL ).amount.value == bob_balance.amount.value );
      ...
      op.expiration = db->head_block_time() + fc::seconds( DPN_MAX_LIMIT_ORDER_EXPIRATION + 1 );
      tx.operations.clear();
      tx.signatures.clear();
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      ...
      BOOST_REQUIRE( limit_order->get_market() == std::make_pair( alice_symbol, DPN_SYMBOL ) );
      BOOST_REQUIRE( db->get_balance( alice_account, alice_symbol ).amount.value == alice_smt_balance.amount.value );
      BOOST_REQUIRE( db->get_balance( alice_account, DPN_SYMBOL ).amount.value == alice_balance.amount.value );
      validate_database();
      ...
      //auto start_dbd = gpo.current_dbd_supply;
...
BOOST_AUTO_TEST_CASE( smt_limit_order_cancel_authorities )
{
   try
   {
      ...
      c.expiration = db->head_block_time() + fc::seconds( DPN_MAX_LIMIT_ORDER_EXPIRATION );
...
BOOST_AUTO_TEST_CASE( smt_limit_order_cancel_apply )
{
   try
   {
      ...
      create.expiration = db->head_block_time() + fc::seconds( DPN_MAX_LIMIT_ORDER_EXPIRATION );
      tx.operations.clear();
      tx.signatures.clear();
...
BOOST_AUTO_TEST_CASE( claim_reward_balance2_validate )
{
   try
   {
      ...
      DPN_REQUIRE_THROW( op.validate(), fc::assert_exception );
...
BOOST_AUTO_TEST_CASE( claim_reward_balance2_apply )
{
   try
   {
      ...
      db_plugin->debug_update( []( database& db )
      {
         ...
         {
            a.reward_dbd_balance = ASSET( "10.000 TBD" );
            a.reward_dpn_balance = ASSET( "10.000 TESTS" );
            a.reward_vesting_balance = ASSET( "10.000000 VESTS" );
            a.reward_vesting_dpn = ASSET( "10.000 TESTS" );
         });

         db.modify( db.get_dynamic_global_properties(), []( dynamic_global_property_object& gpo )
         {
            gpo.current_dbd_supply += ASSET( "10.000 TBD" );
            ...
            gpo.pending_rewarded_vesting_dpn += ASSET( "10.000 TESTS" );
         });
      });
      ...
      auto alice_dpn = db->get_account( "alice" ).balance;
      auto alice_dbd = db->get_account( "alice" ).dbd_balance;
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).balance == alice_dpn + ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).reward_dpn_balance == ASSET( "10.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).dbd_balance == alice_dbd + ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).reward_dbd_balance == ASSET( "10.000 TBD" ) );
      ...
      BOOST_REQUIRE( db->get_account( "alice" ).reward_vesting_dpn == ASSET( "5.000 TESTS" ) );
      validate_database();
      ...
      // Legacy symbols
      asset full_dpn = ASSET( "10.000 TESTS" );
      asset full_dbd = ASSET( "10.000 TBD" );
      op.reward_tokens.push_back( full_dbd );
      op.reward_tokens.push_back( full_dpn );
      op.reward_tokens.push_back( partial_vests );
      PUSH_OP(op, alice_private_key);
      BOOST_REQUIRE( db->get_account( "alice" ).balance == alice_dpn + full_dpn );
      BOOST_REQUIRE( db->get_account( "alice" ).reward_dpn_balance == ASSET( "0.000 TESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).dbd_balance == alice_dbd + full_dbd );
      BOOST_REQUIRE( db->get_account( "alice" ).reward_dbd_balance == ASSET( "0.000 TBD" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).vesting_shares == alice_vests + partial_vests );
      BOOST_REQUIRE( db->get_account( "alice" ).reward_vesting_balance == ASSET( "0.000000 VESTS" ) );
      BOOST_REQUIRE( db->get_account( "alice" ).reward_vesting_dpn == ASSET( "0.000 TESTS" ) );
      validate_database();
      op.reward_tokens.clear();
      // SMTs
...
BOOST_AUTO_TEST_CASE( smt_create_validate )
{
   try
   {
      ...
      BOOST_TEST_MESSAGE( " -- Valid maximum SMT creation fee (DPN_MAX_SHARE_SUPPLY)" );
      op.smt_creation_fee.amount = DPN_MAX_SHARE_SUPPLY;
      op.validate();
      ...
      BOOST_TEST_MESSAGE( " -- Invalid SMT creation symbol: DPN cannot be an SMT" );
      op.symbol = DPN_SYMBOL;
      DPN_REQUIRE_THROW( op.validate(), fc::assert_exception );

      BOOST_TEST_MESSAGE( " -- Invalid SMT creation symbol: DBD cannot be an SMT" );
      op.symbol = DBD_SYMBOL;
      DPN_REQUIRE_THROW( op.validate(), fc::assert_exception );
...
BOOST_AUTO_TEST_CASE( smt_create_duplicate )
{
   try
   {
      ...
      DPN_REQUIRE_THROW( create_smt_with_nai( "alice", alice_private_key, alice_symbol.to_nai(), alice_symbol.decimals() ), fc::assert_exception)
   }
   FC_LOG_AND_RETHROW();
}
...
BOOST_AUTO_TEST_CASE( smt_create_duplicate_differing_decimals )
{
   try
   {
      ...
      // Fail on duplicate SMT lookup
      DPN_REQUIRE_THROW( create_smt_with_nai( "alice", alice_private_key, alice_symbol.to_nai(), 2 /* Decimals */ ), fc::assert_exception)
   }
   FC_LOG_AND_RETHROW();
}
...
BOOST_AUTO_TEST_CASE( smt_create_duplicate_different_users )
{
   try
   {
      ...
      // Fail on duplicate SMT lookup
      DPN_REQUIRE_THROW( create_smt_with_nai( "bob", bob_private_key, alice_symbol.to_nai(), alice_symbol.decimals() ), fc::assert_exception)
   }
   FC_LOG_AND_RETHROW();
}

BOOST_AUTO_TEST_CASE( smt_create_with_dpn_funds )
{
   try
   {
      BOOST_TEST_MESSAGE( "Testing: smt_create_with_dpn_funds" );
      ...
      {
         dgpo.smt_creation_fee = asset( 1000, DBD_SYMBOL );
      } );
...
BOOST_AUTO_TEST_CASE( smt_create_with_dbd_funds )
{
   try
   {
      BOOST_TEST_MESSAGE( "Testing: smt_create_with_dbd_funds" );
...
BOOST_AUTO_TEST_CASE( smt_create_with_invalid_nai )
{
   try
   {
      ...
      DPN_REQUIRE_THROW( create_smt_with_nai( "alice", alice_private_key, ast.to_nai(), ast.decimals() ), fc::assert_exception)
   }
   FC_LOG_AND_RETHROW();
}
...
BOOST_AUTO_TEST_CASE( smt_creation_fee_test )
{
   try
   {
      ...
      // This ensures that our actual smt_creation_fee is sane in production (either DPN or DBD)
      const dynamic_global_property_object& dgpo = db->get_dynamic_global_properties();
      FC_ASSERT( dgpo.smt_creation_fee.symbol == DPN_SYMBOL || dgpo.smt_creation_fee.symbol == DBD_SYMBOL,
      ...
      for ( int i = 0; i < 2; i++ )
      {
         ...
         if ( !i ) // First pass
            ...
            {
               dgpo.smt_creation_fee = asset( 2000, DPN_SYMBOL );
            } );
         else // Second pass
            db->modify( dgpo, [&] ( dynamic_global_property_object& dgpo )
            {
         ...
         BOOST_TEST_MESSAGE( " -- Valid creation fee, using DPN" );
         // We should be able to pay with DPN
         smt_create_operation op;
         op.control_account = "alice";
         ...
         BOOST_TEST_MESSAGE( " -- Valid creation fee, using DBD" );
         // We should be able to pay with DBD
...
BOOST_AUTO_TEST_CASE( smt_create_reset )
{
   try
   {
      ...
      rewards.percent_curation_rewards = 15 * DPN_1_PERCENT;
      ...
      tx.operations.push_back( op );
      sign( tx, alice_private_key );
      DPN_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::assert_exception );
      ...
      BOOST_REQUIRE( token.cashout_window_seconds == DPN_CASHOUT_WINDOW_SECONDS );
      BOOST_REQUIRE( token.reverse_auction_window_seconds == DPN_REVERSE_AUCTION_WINDOW_SECONDS_HF20 );
      BOOST_REQUIRE( token.vote_regeneration_period_seconds == DPN_VOTING_MANA_REGENERATION_SECONDS );
      BOOST_REQUIRE( token.votes_per_regeneration_period == SMT_DEFAULT_VOTES_PER_REGEN_PERIOD );
      BOOST_REQUIRE( token.content_constant == DPN_CONTENT_CONSTANT_HF0 );
      ...
      tx.set_expiration( db->head_block_time() + DPN_BLOCK_INTERVAL * 10 );
      tx.signatures.clear();
      sign( tx, alice_private_key );
      DPN_REQUIRE_THROW( db->push_transaction( tx, 0 ), fc::assert_exception );
   }
...
BOOST_AUTO_TEST_CASE( smt_nai_pool_count )
{
   try
   {
      ...
      DPN_REQUIRE_THROW( get_new_smt_symbol( 0, this->db ), fc::assert_exception );
...
BOOST_AUTO_TEST_CASE( smt_setup_emissions_validate )
{
   try
   {
      ...
      op.symbol = DPN_SYMBOL;
      op.lep_abs_amount = asset( 0, DPN_SYMBOL );
      op.rep_abs_amount = asset( 0, DPN_SYMBOL );
      DPN_REQUIRE_THROW( op.validate(), fc::exception );
      op.symbol = alice_symbol;
      op.lep_abs_amount = asset( 0, alice_symbol );
      op.rep_abs_amount = asset( 0, alice_symbol );
      ...
      op.schedule_time = DPN_GENESIS_TIME;
      DPN_REQUIRE_THROW( op.validate(), fc::exception );
...
BOOST_AUTO_TEST_CASE( set_setup_parameters_validate )
{
   try
   {
      ...
      op.symbol = DPN_SYMBOL;
      DPN_REQUIRE_THROW( op.validate(), fc::assert_exception ); // Invalid symbol
      op.symbol = VESTS_SYMBOL;
      DPN_REQUIRE_THROW( op.validate(), fc::assert_exception ); // Invalid symbol
      op.symbol = DBD_SYMBOL;
      DPN_REQUIRE_THROW( op.validate(), fc::assert_exception ); // Invalid symbol
      ...
      op.control_account = "####";
      DPN_REQUIRE_THROW( op.validate(), fc::assert_exception ); // Invalid account name
      op.control_account = "alice";

      op.setup_parameters.clear();
      DPN_REQUIRE_THROW( op.validate(), fc::assert_exception ); // Empty setup parameters
      op.setup_parameters.emplace( smt_param_allow_voting { .value = true } );
...
BOOST_AUTO_TEST_CASE( smt_set_runtime_parameters_validate )
{
   try
   {
      ...
      smt_param_rewards_v1 rewards;
      rewards.content_constant = DPN_CONTENT_CONSTANT_HF0;
      rewards.percent_curation_rewards = DPN_100_PERCENT + 1;
      ...
      rewards.percent_curation_rewards = DPN_100_PERCENT;
      op.runtime_parameters.clear();
      op.runtime_parameters.insert( rewards );
      op.validate();
...
BOOST_AUTO_TEST_CASE( smt_contribute_validate )
{
   try
   {
      ...
      op.contribution = asset( 1000, DPN_SYMBOL );
      ...
      op.contribution = asset( -1, DPN_SYMBOL );
      DPN_REQUIRE_THROW( op.validate(), fc::assert_exception );
      op.contribution = asset( 1000, DPN_SYMBOL );

      BOOST_TEST_MESSAGE( " -- Failure on no contribution" );
      op.contribution = asset( 0, DPN_SYMBOL );
      DPN_REQUIRE_THROW( op.validate(), fc::assert_exception );
      op.contribution = asset( 1000, DPN_SYMBOL );
      ...
      BOOST_TEST_MESSAGE( " -- Failure on DBD contribution" );
      op.contribution = asset( 1000, DBD_SYMBOL );
      DPN_REQUIRE_THROW( op.validate(), fc::assert_exception );
      op.contribution = asset( 1000, DPN_SYMBOL );
      ...
      BOOST_TEST_MESSAGE( " -- Failure on contribution to DPN" );
      op.symbol = DPN_SYMBOL;
      DPN_REQUIRE_THROW( op.validate(), fc::assert_exception );
      op.symbol = new_symbol;
      ...
      BOOST_TEST_MESSAGE( " -- Failure on contribution to DBD" );
      op.symbol = DBD_SYMBOL;
      DPN_REQUIRE_THROW( op.validate(), fc::assert_exception );
      op.symbol = new_symbol;
...
BOOST_AUTO_TEST_CASE( smt_contribute_apply )
{
   try
   {
      ...
      auto alice_asset_accumulator = asset( 0, DPN_SYMBOL );
      auto bob_asset_accumulator = asset( 0, DPN_SYMBOL );
      auto sam_asset_accumulator = asset( 0, DPN_SYMBOL );
      ...
      db_plugin->debug_update( [=] ( database& db )
      {
         ...
         db.create< smt_ico_object >( [&]( smt_ico_object& o )
         {
            ...
            o.dpn_units_soft_cap = SMT_MIN_SOFT_CAP_DPN_UNITS;
            o.dpn_units_hard_cap = 99000;
         } );
      } );

      smt_contribute_operation bob_op;
      bob_op.contributor = "bob";
      bob_op.contribution = asset( 1000, DPN_SYMBOL );
      ...
      alice_op.contribution = asset( 2000, DPN_SYMBOL );
      ...
      sam_op.contribution = asset( 3000, DPN_SYMBOL );
      ...
      auto alices_contributions = asset( 0, DPN_SYMBOL );
      auto bobs_contributions = asset( 0, DPN_SYMBOL );
      auto sams_contributions = asset( 0, DPN_SYMBOL );
      ...
      BOOST_REQUIRE( db->get_balance( "alice", DPN_SYMBOL ) == ASSET( "1000.000 TESTS" ) - alice_asset_accumulator );
      BOOST_REQUIRE( db->get_balance( "bob", DPN_SYMBOL ) == ASSET( "1000.000 TESTS" ) - bob_asset_accumulator );
      BOOST_REQUIRE( db->get_balance( "sam", DPN_SYMBOL ) == ASSET( "1000.000 TESTS" ) - sam_asset_accumulator );
