...
#include <dpn/chain/hardfork_property_object.hpp>
#include <dpn/chain/node_property_object.hpp>
#include <dpn/chain/notifications.hpp>

#include <dpn/chain/util/advanced_benchmark_dumper.hpp>
#include <dpn/chain/util/signal.hpp>
...
namespace dpn { namespace chain {
   ...
   {
      fc::time_point_sec         times[ DPN_NUM_HARDFORKS + 1 ];
      protocol::hardfork_version versions[ DPN_NUM_HARDFORKS + 1 ];
   };
   ...
   class database : public chainbase::database
   {
      public:
         ...
         struct open_args
         {
            ...
            uint64_t initial_supply = DPN_INIT_SUPPLY;
            uint64_t dbd_initial_supply = DPN_DBD_INIT_SUPPLY;
         ...
         chain_id_type dpn_chain_id = DPN_CHAIN_ID;
      ...
      public:
          ...
          *
          * Passing slot_num == 0 returns DPN_NULL_WITNESS
          */
         ...
         /** @return the dbd created and deposited to_account, may return DPN if there is no median feed */
         std::pair< asset, asset > create_dbd( const account_object& to_account, asset dpn, bool to_reward_balance=false );
         asset create_vesting( const account_object& to_account, asset dpn, bool to_reward_balance=false );
         void adjust_total_payout( const comment_object& a, const asset& dbd, const asset& curator_dbd_value, const asset& beneficiary_value );
         ...
         /**
          * Helper method to return the current dbd value of a given amount of
          * DPN.  Return 0 DBD if there isn't a current_median_history
          */
         asset to_dbd( const asset& dpn )const;
         asset to_dpn( const asset& dbd )const;
         ...
         void init_schema();
         void init_genesis(uint64_t initial_supply = DPN_INIT_SUPPLY, uint64_t dbd_initial_supply = DPN_DBD_INIT_SUPPLY );
