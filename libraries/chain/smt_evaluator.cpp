...
{
   FC_ASSERT( _db.has_hardfork( DPN_SMT_HARDFORK ), "SMT functionality not enabled until hardfork ${hf}", ("hf", DPN_SMT_HARDFORK) );
   ...
   if ( o.smt_creation_fee.amount > 0 ) // Creation case
   {
      ...
      else
      {
         ...
         if( dgpo.smt_creation_fee.symbol == DPN_SYMBOL )
            creation_fee = _db.to_dpn( o.smt_creation_fee );
         else
            creation_fee = _db.to_dbd( o.smt_creation_fee );
      }
      ...
      _db.adjust_balance( DPN_NULL_ACCOUNT,  o.smt_creation_fee );
   }
   else // Reset case
   {
...
#pragma message ("TODO: Add/modify test to check the token phase correctly set.")
   ...
   auto token_ico = _db.create< smt_ico_object >( [&] ( smt_ico_object& token_ico_obj )
   {
      ...
      token_ico_obj.dpn_units_soft_cap = o.dpn_units_soft_cap;
      token_ico_obj.dpn_units_hard_cap = o.dpn_units_hard_cap;
   } );
...
void smt_contribute_evaluator::do_apply( const smt_contribute_operation& o )
{
   try
   {
      ...
      FC_ASSERT( token_ico->contributed.amount < token_ico->dpn_units_hard_cap, "SMT ICO has reached its hard cap and no longer accepts contributions" );
      FC_ASSERT( token_ico->contributed.amount + o.contribution.amount <= token_ico->dpn_units_hard_cap,
         "The proposed contribution would exceed the ICO hard cap, maximum possible contribution: ${c}",
         ("c", asset( token_ico->dpn_units_hard_cap - token_ico->contributed.amount, DPN_SYMBOL )) );
