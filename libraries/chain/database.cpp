...
#include <dpn/chain/custom_operation_interpreter.hpp>
#include <dpn/chain/database.hpp>
#include <dpn/chain/database_exceptions.hpp>
#include <dpn/chain/db_with.hpp>
#include <dpn/chain/evaluator_registry.hpp>
#include <dpn/chain/global_property_object.hpp>
#include <dpn/chain/history_object.hpp>
#include <dpn/chain/optional_action_evaluator.hpp>
...
#include <dpn/chain/shared_db_merkle.hpp>
...
#include <dpn/chain/util/nai_generator.hpp>
...
FC_REFLECT( dpn::chain::object_schema_repr, (space_type)(type) )
FC_REFLECT( dpn::chain::operation_schema_repr, (id)(type) )
FC_REFLECT( dpn::chain::db_schema, (types)(object_types)(operation_type)(custom_operation_types) )
...
struct reward_fund_context
{
   ...
   asset       reward_balance = asset( 0, DPN_SYMBOL );
   share_type  dpn_awarded = 0;
};
...
void database::open( const open_args& args )
{
   try
   {
      ...
      if( !find< dynamic_global_property_object >() )
         with_write_lock( [&]()
         {
            init_genesis( args.initial_supply, args.dbd_initial_supply );
         });
...
uint32_t database::reindex( const open_args& args )
{
   ...
   BOOST_SCOPE_EXIT(this_,&note) {
      DPN_TRY_NOTIFY(this_->_post_reindex_signal, note);
   ...
   try
   {
      ...
      DPN_TRY_NOTIFY(_pre_reindex_signal, note);
      ...
      DPN_ASSERT( _block_log.head(), block_log_exception, "No blocks in block log. Cannot reindex an empty chain." );
...
{ try {
   if( !has_hardfork( DPN_HARDFORK_0_6__127 ) )
...
{
   if( has_hardfork( DPN_HARDFORK_0_17__769 ) || comment.parent_author == DPN_ROOT_POST_PARENT )
...
{
   return get< reward_fund_object, by_name >( DPN_POST_REWARD_FUND_NAME );
}
...
uint32_t database::witness_participation_rate()const
{
   ...
   return uint64_t(DPN_100_PERCENT) * dpo.recent_slots_filled.popcount() / 128;
}
...
void database::pop_block()
{
   try
   {
      ...
      DPN_ASSERT( head_block.valid(), pop_empty_chain, "there are no blocks to pop" );
...
{
   DPN_TRY_NOTIFY( _pre_apply_operation_signal, note )
}
...
fc::time_point_sec database::get_slot_time(uint32_t slot_num)const
{
   ...
   auto interval = DPN_BLOCK_INTERVAL;
   const dynamic_global_property_object& dpo = get_dynamic_global_properties();
...
uint32_t database::get_slot_at_time(fc::time_point_sec when)const
{
   ...
   if( when < first_slot_time )
      return 0;
   return (when - first_slot_time).to_seconds() / DPN_BLOCK_INTERVAL + 1;
}

/**
 *  Converts DPN into dbd and adds it to to_account while reducing the DPN supply
 *  by DPN and increasing the dbd supply by the specified amount.
 */
std::pair< asset, asset > database::create_dbd( const account_object& to_account, asset dpn, bool to_reward_balance )
{
   std::pair< asset, asset > assets( asset( 0, DBD_SYMBOL ), asset( 0, DPN_SYMBOL ) );
   ...
   try
   {
      ...
      {
         auto to_dbd = ( gpo.dbd_print_rate * dpn.amount ) / DPN_100_PERCENT;
         auto to_dpn = dpn.amount - to_dbd;

         auto dbd = asset( to_dbd, DPN_SYMBOL ) * median_price;

         if( to_reward_balance )
         {
            adjust_reward_balance( to_account, dbd );
            adjust_reward_balance( to_account, asset( to_dpn, DPN_SYMBOL ) );
         }
         else
         {
            adjust_balance( to_account, dbd );
            adjust_balance( to_account, asset( to_dpn, DPN_SYMBOL ) );
         }

         adjust_supply( asset( -to_dbd, DPN_SYMBOL ) );
         adjust_supply( dbd );
         assets.first = dbd;
         assets.second = asset( to_dpn, DPN_SYMBOL );
      }
      else
      {
         adjust_balance( to_account, dpn );
         assets.second = dpn;
      }
   }
   FC_CAPTURE_LOG_AND_RETHROW( (to_account.name)(dpn) )
...
asset create_vesting2( database& db, const account_object& to_account, asset liquid, bool to_reward_balance, Before&& before_vesting_callback )
{
   try
   {
      auto calculate_new_vesting = [ liquid ] ( price vesting_share_price ) -> asset
         {
         /**
          *  The ratio of total_vesting_shares / total_vesting_fund_dpn should not
      ...
      FC_ASSERT( liquid.symbol == DPN_SYMBOL );
      // ^ A novelty, needed but risky in case someone managed to slip DBD/TESTS here in blockchain history.
      // Get share price.
      ...
      {
         if( db.has_hardfork( DPN_HARDFORK_0_20__2539 ) )
         {
            db.modify( to_account, [&]( account_object& a )
            {
               util::update_manabar(
                  cprops,
                  a,
                  db.has_hardfork( DPN_HARDFORK_0_21__3336 ),
      ...
      db.modify( cprops, [&]( dynamic_global_property_object& props )
      {
         if( to_reward_balance )
         {
            ...
            props.pending_rewarded_vesting_dpn += liquid;
         }
         else
         {
            props.total_vesting_fund_dpn += liquid;
 ...
 * @param liquid     - DPN or liquid SMT to be converted to vesting shares
 */
...
uint32_t database::get_pow_summary_target()const
{
   ...
   if( has_hardfork( DPN_HARDFORK_0_16__551 ) )
...
void database::adjust_proxied_witness_votes( const account_object& a,
                                   const std::array< share_type, DPN_MAX_PROXY_RECURSION_DEPTH+1 >& delta,
                                   int depth )
{
   if( a.proxy != DPN_PROXY_TO_SELF_ACCOUNT )
   {
      /// nested proxies are not supported, vote will not propagate
      if( depth >= DPN_MAX_PROXY_RECURSION_DEPTH )
         return;
      ...
      {
         for( int i = DPN_MAX_PROXY_RECURSION_DEPTH - depth - 1; i >= 0; --i )
         {
   ...
   else
   {
      ...
      for( int i = DPN_MAX_PROXY_RECURSION_DEPTH - depth; i >= 0; --i )
         total_delta += delta[i];
...
void database::adjust_witness_vote( const witness_object& witness, share_type delta )
{
   ...
   modify( witness, [&]( witness_object& w )
   {
      ...
      if( has_hardfork( DPN_HARDFORK_0_2 ) )
         w.virtual_scheduled_time = w.virtual_last_update + (DPN_VIRTUAL_SCHEDULE_LAP_LENGTH2 - w.virtual_position)/(w.votes.value+1);
      else
         w.virtual_scheduled_time = w.virtual_last_update + (DPN_VIRTUAL_SCHEDULE_LAP_LENGTH - w.virtual_position)/(w.votes.value+1);

      /** witnesses with a low number of votes could overflow the time field and end up with a scheduled time in the past */
      if( has_hardfork( DPN_HARDFORK_0_4 ) )
      {
...
void database::clear_witness_votes( const account_object& a )
{
   ...
   if( has_hardfork( DPN_HARDFORK_0_6__104 ) )
...
{
   if( !has_hardfork( DPN_HARDFORK_0_14__327 ) ) return;

   const auto& null_account = get_account( DPN_NULL_ACCOUNT );
   asset total_dpn( 0, DPN_SYMBOL );
   asset total_dbd( 0, DBD_SYMBOL );
   asset total_vests( 0, VESTS_SYMBOL );

   asset vesting_shares_dpn_value = asset( 0, DPN_SYMBOL );

   if( null_account.balance.amount > 0 )
   {
      total_dpn += null_account.balance;
   }

   if( null_account.savings_balance.amount > 0 )
   {
      total_dpn += null_account.savings_balance;
   }

   if( null_account.dbd_balance.amount > 0 )
   {
      total_dbd += null_account.dbd_balance;
   }

   if( null_account.savings_dbd_balance.amount > 0 )
   {
      total_dbd += null_account.savings_dbd_balance;
   }

   if( null_account.vesting_shares.amount > 0 )
   {
      const auto& gpo = get_dynamic_global_properties();
      vesting_shares_dpn_value = null_account.vesting_shares * gpo.get_vesting_share_price();
      total_dpn += vesting_shares_dpn_value;
      total_vests += null_account.vesting_shares;
   }

   if( null_account.reward_dpn_balance.amount > 0 )
   {
      total_dpn += null_account.reward_dpn_balance;
   }

   if( null_account.reward_dbd_balance.amount > 0 )
   {
      total_dbd += null_account.reward_dbd_balance;
   }

   if( null_account.reward_vesting_balance.amount > 0 )
   {
      total_dpn += null_account.reward_vesting_dpn;
      total_vests += null_account.reward_vesting_balance;
   }

   if( (total_dpn.amount.value == 0) && (total_dbd.amount.value == 0) && (total_vests.amount.value == 0) )
      return;
   ...
   if( total_dpn.amount.value > 0 )
      vop.total_cleared.push_back( total_dpn );
   if( total_vests.amount.value > 0 )
      vop.total_cleared.push_back( total_vests );
   if( total_dbd.amount.value > 0 )
      vop.total_cleared.push_back( total_dbd );
   ...
   if( null_account.dbd_balance.amount > 0 )
   {
      adjust_balance( null_account, -null_account.dbd_balance );
   }

   if( null_account.savings_dbd_balance.amount > 0 )
   {
      adjust_savings_balance( null_account, -null_account.savings_dbd_balance );
   }
   ...
   if( null_account.vesting_shares.amount > 0 )
   {
      ...
      modify( gpo, [&]( dynamic_global_property_object& g )
      {
         ...
         g.total_vesting_fund_dpn -= vesting_shares_dpn_value;
      });
   ...
   if( null_account.reward_dpn_balance.amount > 0 )
   {
      adjust_reward_balance( null_account, -null_account.reward_dpn_balance );
   }

   if( null_account.reward_dbd_balance.amount > 0 )
   {
      adjust_reward_balance( null_account, -null_account.reward_dbd_balance );
   }
   ...
   if( null_account.reward_vesting_balance.amount > 0 )
   {
      ...
      modify( gpo, [&]( dynamic_global_property_object& g )
      {
         ...
         g.pending_rewarded_vesting_dpn -= null_account.reward_vesting_dpn;
      });

      modify( null_account, [&]( account_object& a )
      {
         a.reward_vesting_dpn.amount = 0;
   ...
   if( total_dpn.amount > 0 )
      adjust_supply( -total_dpn );

   if( total_dbd.amount > 0 )
      adjust_supply( -total_dbd );
...
{
   if( has_hardfork( DPN_PROPOSALS_HARDFORK ) )
   {
...
{
   if( head_block_num() >= DPN_OWNER_AUTH_HISTORY_TRACKING_START_BLOCK_NUM )
   {
...
void database::process_vesting_withdrawals()
{
   ...
   while( current != widx.end() && current->next_vesting_withdrawal <= head_block_time() )
   {
      ...
      */
      ...
      share_type vests_deposited_as_dpn = 0;
      share_type vests_deposited_as_vests = 0;
      asset total_dpn_converted = asset( 0, DPN_SYMBOL );

      // Do two passes, the first for vests, the second for dpn. Try to maintain as much accuracy for vests as possible.
           ...
           ++itr )
      {
         if( itr->auto_vest )
         {
            share_type to_deposit = ( ( fc::uint128_t ( to_withdraw.value ) * itr->percent ) / DPN_100_PERCENT ).to_uint64();
           ...
           ++itr )
      {
         if( !itr->auto_vest )
         {
            ...
            share_type to_deposit = ( ( fc::uint128_t ( to_withdraw.value ) * itr->percent ) / DPN_100_PERCENT ).to_uint64();
            vests_deposited_as_dpn += to_deposit;
            auto converted_dpn = asset( to_deposit, VESTS_SYMBOL ) * cprops.get_vesting_share_price();
            total_dpn_converted += converted_dpn;

            if( to_deposit > 0 )
            {
               operation vop = fill_vesting_withdraw_operation( from_account.name, to_account.name, asset( to_deposit, VESTS_SYMBOL), converted_dpn );

               pre_push_virtual_operation( vop );

               modify( to_account, [&]( account_object& a )
               {
                  a.balance += converted_dpn;
               });

               modify( cprops, [&]( dynamic_global_property_object& o )
               {
                  o.total_vesting_fund_dpn -= converted_dpn;
      ...
      share_type to_convert = to_withdraw - vests_deposited_as_dpn - vests_deposited_as_vests;
      FC_ASSERT( to_convert >= 0, "Deposited more vests than were supposed to be withdrawn" );

      auto converted_dpn = asset( to_convert, VESTS_SYMBOL ) * cprops.get_vesting_share_price();
      operation vop = fill_vesting_withdraw_operation( from_account.name, from_account.name, asset( to_convert, VESTS_SYMBOL ), converted_dpn );
      pre_push_virtual_operation( vop );
      ...
      modify( from_account, [&]( account_object& a )
      {
         ...
         {
            a.next_vesting_withdrawal += fc::seconds( DPN_VESTING_WITHDRAW_INTERVAL_SECONDS );
         }
      });
...
void database::adjust_total_payout( const comment_object& cur, const asset& dbd_created, const asset& curator_dbd_value, const asset& beneficiary_value )
{
   modify( cur, [&]( comment_object& c )
   {
      // input assets should be in dbd
      c.total_payout_value += dbd_created;
      c.curator_payout_value += curator_dbd_value;
      ...
      else if( c.total_vote_weight > 0 )
      {
         ...
         for( auto& item : proxy_set )
         {
            ...
            if( claim > 0 ) // min_amt is non-zero satoshis
            {
               ...
               create_vesting2( *this, voter, asset( claim, DPN_SYMBOL ), has_hardfork( DPN_HARDFORK_0_17__659 ),
                  [&]( const asset& reward )
                  {
...
void fill_comment_reward_context_local_state( util::comment_reward_context& ctx, const comment_object& comment )
{
   ...
   ctx.max_dbd = comment.max_accepted_payout;
}
...
share_type database::cashout_comment_helper( util::comment_reward_context& ctx, const comment_object& comment, bool forward_curation_remainder )
{
   try
   {
      ...
      if( comment.net_rshares > 0 )
      {
         ...
         if( has_hardfork( DPN_HARDFORK_0_17__774 ) )
         {
         ...
         if( reward_tokens > 0 )
         {
            share_type curation_tokens = ( ( reward_tokens * get_curation_rewards_percent( comment ) ) / DPN_100_PERCENT ).to_uint64();
            ...
            {
               auto benefactor_tokens = ( author_tokens * b.weight ) / DPN_100_PERCENT;
               auto benefactor_vesting_dpn = benefactor_tokens;
               auto vop = comment_benefactor_reward_operation( b.account, comment.author, to_string( comment.permlink ), asset( 0, DBD_SYMBOL ), asset( 0, DPN_SYMBOL ), asset( 0, VESTS_SYMBOL ) );

               if( has_hardfork( DPN_HARDFORK_0_21__3343 ) && b.account == DPN_TREASURY_ACCOUNT )
               {
                  benefactor_vesting_dpn = 0;
                  vop.dbd_payout = asset( benefactor_tokens, DPN_SYMBOL ) * get_feed_history().current_median_history;
                  adjust_balance( get_account( DPN_TREASURY_ACCOUNT ), vop.dbd_payout );
                  adjust_supply( asset( -benefactor_tokens, DPN_SYMBOL ) );
                  adjust_supply( vop.dbd_payout );
               }
               else if( has_hardfork( DPN_HARDFORK_0_20__2022 ) )
               {
                  auto benefactor_dbd_dpn = ( benefactor_tokens * comment.percent_dpn_dollars ) / ( 2 * DPN_100_PERCENT ) ;
                  benefactor_vesting_dpn  = benefactor_tokens - benefactor_dbd_dpn;
                  auto dbd_payout           = create_dbd( get_account( b.account ), asset( benefactor_dbd_dpn, DPN_SYMBOL ), true );

                  vop.dbd_payout   = dbd_payout.first; // DBD portion
                  vop.dpn_payout = dbd_payout.second; // DPN portion
               }

               create_vesting2( *this, get_account( b.account ), asset( benefactor_vesting_dpn, DPN_SYMBOL ), has_hardfork( DPN_HARDFORK_0_17__659 ),
               [&]( const asset& reward )
               {
            ...
            auto dbd_dpn     = ( author_tokens * comment.percent_dpn_dollars ) / ( 2 * DPN_100_PERCENT ) ;
            auto vesting_dpn = author_tokens - dbd_dpn;

            const auto& author = get_account( comment.author );
            auto dbd_payout = create_dbd( author, asset( dbd_dpn, DPN_SYMBOL ), has_hardfork( DPN_HARDFORK_0_17__659 ) );
            operation vop = author_reward_operation( comment.author, to_string( comment.permlink ), dbd_payout.first, dbd_payout.second, asset( 0, VESTS_SYMBOL ) );

            create_vesting2( *this, author, asset( vesting_dpn, DPN_SYMBOL ), has_hardfork( DPN_HARDFORK_0_17__659 ),
            ...
            adjust_total_payout( comment, dbd_payout.first + to_dbd( dbd_payout.second + asset( vesting_dpn, DPN_SYMBOL ) ), to_dbd( asset( curation_tokens, DPN_SYMBOL ) ), to_dbd( asset( total_beneficiary, DPN_SYMBOL ) ) );

            post_push_virtual_operation( vop );
            vop = comment_reward_operation( comment.author, to_string( comment.permlink ), to_dbd( asset( claimed_reward, DPN_SYMBOL ) ) );
            pre_push_virtual_operation( vop );
            post_push_virtual_operation( vop );
      ...
      modify( comment, [&]( comment_object& c )
      {
         ...
         */
         ...
         if( has_hardfork( DPN_HARDFORK_0_17__769 ) )
         {
            c.cashout_time = fc::time_point_sec::maximum();
         }
         else if( c.parent_author == DPN_ROOT_POST_PARENT )
         {
            if( has_hardfork( DPN_HARDFORK_0_12__177 ) && c.last_payout == fc::time_point_sec::min() )
               c.cashout_time = head_block_time() + DPN_SECOND_CASHOUT_WINDOW;
            else
               c.cashout_time = fc::time_point_sec::maximum();
         }
      ...
      while( vote_itr != vote_idx.end() && vote_itr->comment == comment.id )
      {
         ...
         ++vote_itr;
         if( !has_hardfork( DPN_HARDFORK_0_12__177 ) || calculate_discussion_payout_time( comment ) != fc::time_point_sec::maximum() )
         {
...
void database::process_comment_cashout()
{
   ...
   if( !has_hardfork( DPN_FIRST_CASHOUT_TIME ) )
      return;

   const auto& gpo = get_dynamic_global_properties();
   util::comment_reward_context ctx;
   ctx.current_dpn_price = get_feed_history().current_median_history;

   vector< reward_fund_context > funds;
   vector< share_type > dpn_awarded;
   ...
   for( auto itr = reward_idx.begin(); itr != reward_idx.end(); ++itr )
   {
      ...
      modify( *itr, [&]( reward_fund_object& rfo )
      {
         ...
         if( has_hardfork( DPN_HARDFORK_0_19__1051 ) )
            decay_time = DPN_RECENT_RSHARES_DECAY_TIME_HF19;
         else
            decay_time = DPN_RECENT_RSHARES_DECAY_TIME_HF17;
   ...
   if( has_hardfork( DPN_HARDFORK_0_17__771 ) )
   {
    ...
    * The helper only does token allocation based on curation rewards and the DBD
    * global %, etc.
    *
   ...
   while( current != cidx.end() && current->cashout_time <= head_block_time() )
   {
      if( has_hardfork( DPN_HARDFORK_0_17__771 ) )
      {
         ...
         ctx.total_reward_fund_dpn = funds[ fund_id ].reward_balance;

         bool forward_curation_remainder = !has_hardfork( DPN_HARDFORK_0_20__1877 );

         funds[ fund_id ].dpn_awarded += cashout_comment_helper( ctx, *current, forward_curation_remainder );
      }
      else
      {
         ...
         while( itr != com_by_root.end() && itr->root_comment == current->root_comment )
         {
            ...
            ctx.total_reward_fund_dpn = gpo.total_reward_fund_dpn;
            ...
            if( reward > 0 )
            {
               ...
               {
                  p.total_reward_fund_dpn.amount -= reward;
               });
            }
         }
      }
   ...
   if( funds.size() )
   {
      for( size_t i = 0; i < funds.size(); i++ )
      {
         modify( get< reward_fund_object, by_id >( reward_fund_id_type( i ) ), [&]( reward_fund_object& rfo )
         {
            ...
            rfo.reward_balance -= asset( funds[ i ].dpn_awarded, DPN_SYMBOL );
         });
      }
   }
}
...
/**
 *  Overall the network has an inflation rate of 102% of virtual dpn per year
...
void database::process_funds()
{
   ...
   if( has_hardfork( DPN_HARDFORK_0_16__551) )
   {
       ...
       */
      int64_t start_inflation_rate = int64_t( DPN_INFLATION_RATE_START_PERCENT );
      int64_t inflation_rate_adjustment = int64_t( head_block_num() / DPN_INFLATION_NARROWING_PERIOD );
      int64_t inflation_rate_floor = int64_t( DPN_INFLATION_RATE_STOP_PERCENT );

      // below subtraction cannot underflow int64_t because inflation_rate_adjustment is <2^32
      int64_t current_inflation_rate = std::max( start_inflation_rate - inflation_rate_adjustment, inflation_rate_floor );

      auto new_dpn = ( props.virtual_supply.amount * current_inflation_rate ) / ( int64_t( DPN_100_PERCENT ) * int64_t( DPN_BLOCKS_PER_YEAR ) );
      auto content_reward = ( new_dpn * props.content_reward_percent ) / DPN_100_PERCENT;
      if( has_hardfork( DPN_HARDFORK_0_17__774 ) )
         content_reward = pay_reward_funds( content_reward );
      auto vesting_reward = ( new_dpn * props.vesting_reward_percent ) / DPN_100_PERCENT;
      auto sps_fund = ( new_dpn * props.sps_fund_percent ) / DPN_100_PERCENT;
      auto witness_reward = new_dpn - content_reward - vesting_reward - sps_fund;

      const auto& cwit = get_witness( props.current_witness );
      witness_reward *= DPN_MAX_WITNESSES;
      ...
      auto new_dbd = asset( 0, DBD_SYMBOL );

      if( sps_fund.value )
      {
         new_dbd = asset( sps_fund, DPN_SYMBOL ) * feed.current_median_history;
         adjust_balance( DPN_TREASURY_ACCOUNT, new_dbd );
      }

      new_dpn = content_reward + vesting_reward + witness_reward;

      modify( props, [&]( dynamic_global_property_object& p )
      {
         p.total_vesting_fund_dpn += asset( vesting_reward, DPN_SYMBOL );
         if( !has_hardfork( DPN_HARDFORK_0_17__774 ) )
            p.total_reward_fund_dpn  += asset( content_reward, DPN_SYMBOL );
         p.current_supply      += asset( new_dpn, DPN_SYMBOL );
         p.current_dbd_supply  += new_dbd;
         p.virtual_supply      += asset( new_dpn + sps_fund, DPN_SYMBOL );
         p.sps_interval_ledger += new_dbd;
      });

      operation vop = producer_reward_operation( cwit.owner, asset( 0, VESTS_SYMBOL ) );
      create_vesting2( *this, get_account( cwit.owner ), asset( witness_reward, DPN_SYMBOL ), false,
   ...
   else
   {
      ...
      if( props.head_block_number < DPN_START_VESTING_BLOCK )
      ...
      modify( props, [&]( dynamic_global_property_object& p )
      {
          p.total_vesting_fund_dpn += vesting_reward;
          p.total_reward_fund_dpn  += content_reward;
...
{
   if( has_hardfork( DPN_HARDFORK_0_12__178 ) )
      return asset( 0, DPN_SYMBOL );

   const auto& props = get_dynamic_global_properties();
   static_assert( DPN_LIQUIDITY_REWARD_PERIOD_SEC == 60*60, "this code assumes a 1 hour time interval" );
   asset percent( protocol::calc_percent_reward_per_hour< DPN_LIQUIDITY_APR_PERCENT >( props.virtual_supply.amount ), DPN_SYMBOL );
   return std::max( percent, DPN_MIN_LIQUIDITY_REWARD );
}
...
asset database::get_producer_reward()
{
   ...
   if( props.head_block_number >= DPN_START_MINER_VOTING_BLOCK || (witness_account.vesting_shares.amount.value == 0) )
   {
...
#ifndef IS_TEST_NET
   /// 0 block rewards until at least DPN_MAX_WITNESSES have produced a POW
   if( props.num_pow_witnesses < DPN_MAX_WITNESSES && props.head_block_number < DPN_START_VESTING_BLOCK )
      return asset( 0, DPN_SYMBOL );
#endif

   static_assert( DPN_BLOCK_INTERVAL == 1, "this code assumes a 1-second time interval" );
   static_assert( DPN_MAX_WITNESSES == 63, "this code assumes 63 per round" );
...
void database::pay_liquidity_reward()
{
   ...
   if( (head_block_num() % DPN_LIQUIDITY_REWARD_BLOCKS) == 0 )
   {
...
uint16_t database::get_curation_rewards_percent( const comment_object& c ) const
{
   ...
   else if( has_hardfork( DPN_HARDFORK_0_8__116 ) )
      return DPN_1_PERCENT * 25;
   else
      return DPN_1_PERCENT * 50;
}
...
share_type database::pay_reward_funds( share_type reward )
{
   ...
   for( auto itr = reward_idx.begin(); itr != reward_idx.end(); ++itr )
   {
      ...
      auto r = ( reward * itr->percent_content_rewards ) / DPN_100_PERCENT;

      modify( *itr, [&]( reward_fund_object& rfo )
      {
         rfo.reward_balance += asset( r, DPN_SYMBOL );
      });

      used_rewards += r;

      // Sanity check to ensure we aren't printing more DPN than has been allocated through inflation
 ...
 *  the head block time and then converts them to/from dpn/dbd at the
...
void database::process_conversions()
{
   ...
   asset net_dbd( 0, DBD_SYMBOL );
   asset net_dpn( 0, DPN_SYMBOL );
   ...
   modify( props, [&]( dynamic_global_property_object& p )
   {
       p.current_supply += net_dpn;
       p.current_dbd_supply -= net_dbd;
       p.virtual_supply += net_dpn;
       p.virtual_supply -= net_dbd * get_feed_history().current_median_history;
   } );
}

asset database::to_dbd( const asset& dpn )const
{
   return util::to_dbd( get_feed_history().current_median_history, dpn );
}

asset database::to_dpn( const asset& dbd )const
{
   return util::to_dpn( get_feed_history().current_median_history, dbd );
}
...
void database::account_recovery_processing()
{
   ...
   while( hist != hist_idx.end() && time_point_sec( hist->last_valid_time + DPN_OWNER_AUTH_RECOVERY_PERIOD ) < head_block_time() )
   {
      remove( *hist );
      hist = hist_idx.begin();
   }
...
void database::expire_escrow_ratification()
{
   ...
   while( escrow_itr != escrow_idx.end() && !escrow_itr->is_approved() && escrow_itr->ratification_deadline <= head_block_time() )
   {
      ...
      adjust_balance( old_escrow.from, old_escrow.dpn_balance );
      adjust_balance( old_escrow.from, old_escrow.dbd_balance );
...
void database::process_decline_voting_rights()
{
   ...
   while( itr != request_idx.end() && itr->effective_date <= head_block_time() )
   {
      ...
      std::array<share_type, DPN_MAX_PROXY_RECURSION_DEPTH+1> delta;
      delta[0] = -account.vesting_shares.amount;
      for( int i = 0; i < DPN_MAX_PROXY_RECURSION_DEPTH; ++i )
      ...
      {
         a.can_vote = false;
         a.proxy = DPN_PROXY_TO_SELF_ACCOUNT;
      });
...
void database::init_genesis( uint64_t init_supply, uint64_t dbd_init_supply )
{
   try
   {
      ...
      public_key_type      init_public_key(DPN_INIT_PUBLIC_KEY);

      create< account_object >( [&]( account_object& a )
      {
         a.name = DPN_MINER_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = DPN_MINER_ACCOUNT;
      ...
      create< account_object >( [&]( account_object& a )
      {
         a.name = DPN_NULL_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = DPN_NULL_ACCOUNT;
         auth.owner.weight_threshold = 1;
         auth.active.weight_threshold = 1;
      });
      ...
      create< account_object >( [&]( account_object& a )
      {
         a.name = DPN_TREASURY_ACCOUNT;
      } );
#endif

      create< account_object >( [&]( account_object& a )
      {
         a.name = DPN_TEMP_ACCOUNT;
      } );
      create< account_authority_object >( [&]( account_authority_object& auth )
      {
         auth.account = DPN_TEMP_ACCOUNT;
         auth.owner.weight_threshold = 0;
         auth.active.weight_threshold = 0;
      });

      for( int i = 0; i < DPN_NUM_INIT_MINERS; ++i )
      {
         create< account_object >( [&]( account_object& a )
         {
            a.name = DPN_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() );
            a.memo_key = init_public_key;
            a.balance  = asset( i ? 0 : init_supply, DPN_SYMBOL );
            a.dbd_balance = asset( i ? 0 : dbd_init_supply, DBD_SYMBOL );
         } );

         create< account_authority_object >( [&]( account_authority_object& auth )
         {
            auth.account = DPN_INIT_MINER_NAME + ( i ? fc::to_string( i ) : std::string() );
         ...
         {
            w.owner        = DPN_INIT_MINER_NAME + ( i ? fc::to_string(i) : std::string() );
      ...
      {
         p.current_witness = DPN_INIT_MINER_NAME;
         p.time = DPN_GENESIS_TIME;
         p.recent_slots_filled = fc::uint128::max_value();
         p.participation_count = 128;
         p.current_supply = asset( init_supply, DPN_SYMBOL );
         p.init_dbd_supply = asset( dbd_init_supply, DBD_SYMBOL );
         p.virtual_supply = p.current_supply;
         p.maximum_block_size = DPN_MAX_BLOCK_SIZE;
         p.reverse_auction_seconds = DPN_REVERSE_AUCTION_WINDOW_SECONDS_HF6;
         p.dbd_stop_percent = DPN_DBD_STOP_PERCENT_HF14;
         p.dbd_start_percent = DPN_DBD_START_PERCENT_HF14;
         p.dbd_stop_adjust = 0;
         p.next_maintenance_time = DPN_GENESIS_TIME;
         p.last_budget_time = DPN_GENESIS_TIME;
      } );

#ifdef IS_TEST_NET
      create< feed_history_object >( [&]( feed_history_object& o )
      {
         o.current_median_history = price( asset( 1, DBD_SYMBOL ), asset( 1, DPN_SYMBOL ) );
      });
#else
      // Nothing to do
      create< feed_history_object >( [&]( feed_history_object& o ) {});
      ...
      {
         hpo.processed_hardforks.push_back( DPN_GENESIS_TIME );
      } );
      ...
      create< witness_schedule_object >( [&]( witness_schedule_object& wso )
      {
         ...
         wso.current_shuffled_witnesses[0] = DPN_INIT_MINER_NAME;
...
void database::notify_changed_objects()
{
   try
   {
      ...
      get_changed_ids( ids );
      DPN_TRY_NOTIFY( changed_objects, ids )*/
      /*
      if( _undo_db.enabled() )
      {
         ...
         DPN_TRY_NOTIFY( changed_objects, changed_ids )
      }
      */
   }
   FC_CAPTURE_AND_RETHROW()
...
void database::check_free_memory( bool force_print, uint32_t current_block_num )
{
   ...
   if( BOOST_UNLIKELY( _shared_file_full_threshold != 0 && _shared_file_scale_rate != 0 && free_mem < ( ( uint128_t( DPN_100_PERCENT - _shared_file_full_threshold ) * max_mem ) / DPN_100_PERCENT ).to_uint64() ) )
   {
      uint64_t new_max = ( uint128_t( max_mem * _shared_file_scale_rate ) / DPN_100_PERCENT ).to_uint64() + max_mem;
...
{ try {
   ...
   if( has_hardfork( DPN_HARDFORK_0_12 ) )
   {
   ...
   if( block_size < DPN_MIN_BLOCK_SIZE )
   {
      elog( "Block size is too small",
         ("next_block_num",next_block_num)("block_size", block_size)("min",DPN_MIN_BLOCK_SIZE)
      );
   }
   ...
   if( has_hardfork( DPN_HARDFORK_0_5__54 ) ) // Cannot remove after hardfork
   {
...
struct process_header_visitor
{
   ...
   {
      FC_ASSERT( _db.has_hardfork( DPN_SMT_HARDFORK ), "Automated actions are not enabled until SMT hardfork." );
...
try {
   if( (head_block_num() % DPN_FEED_INTERVAL_BLOCKS) != 0 )
      return;
   ...
   for( int i = 0; i < wso.num_scheduled_witnesses; i++ )
   {
      ...
      if( has_hardfork( DPN_HARDFORK_0_19__822 ) )
      {
         if( now < wit.last_dbd_exchange_update + DPN_MAX_FEED_AGE_SECONDS
            && !wit.dbd_exchange_rate.is_null() )
         {
            feeds.push_back( wit.dbd_exchange_rate );
         }
      }
      else if( wit.last_dbd_exchange_update < now + DPN_MAX_FEED_AGE_SECONDS &&
          !wit.dbd_exchange_rate.is_null() )
      {
         feeds.push_back( wit.dbd_exchange_rate );
      }
   ...
   if( feeds.size() >= DPN_MIN_FEEDS )
   {
      ...
      modify( get_feed_history(), [&]( feed_history_object& fho )
      {
         ...
         size_t dpn_feed_history_window = DPN_FEED_HISTORY_WINDOW_PRE_HF_16;
         if( has_hardfork( DPN_HARDFORK_0_16__551) )
            dpn_feed_history_window = DPN_FEED_HISTORY_WINDOW;

         if( fho.price_history.size() > dpn_feed_history_window )
         ...
         if( fho.price_history.size() )
         {
            ...
            if( skip_price_feed_limit_check )
               return;
#endif
            if( has_hardfork( DPN_HARDFORK_0_14__230 ) )
            {
               // This block limits the effective median price to force DBD to remain at or
               // below 10% of the combined market cap of DPN and DBD.
               //
               // For example, if we have 500 DPN and 100 DBD, the price is limited to
               // 900 DBD / 500 DPN which works out to be $1.80.  At this price, 500 Dpn
               // would be valued at 500 * $1.80 = $900.  100 DBD is by definition always $100,
               // so the combined market cap is $900 + $100 = $1000.

               const auto& gpo = get_dynamic_global_properties();

               if( gpo.current_dbd_supply.amount > 0 )
               {
                  price min_price( asset( 9 * gpo.current_dbd_supply.amount, DBD_SYMBOL ), gpo.current_supply );
...
{ try {
   ...
   if( !(skip & (skip_transaction_signatures | skip_authority_check) ) )
   {
      ...
      try
      {
         trx.verify_authority( chain_id, get_active, get_owner, get_posting, DPN_MAX_SIG_CHECK_DEPTH,
            has_hardfork( DPN_HARDFORK_0_20 ) || is_producing() ? DPN_MAX_AUTHORITY_MEMBERSHIP : 0,
            has_hardfork( DPN_HARDFORK_0_20 ) || is_producing() ? DPN_MAX_SIG_CHECK_ACCOUNTS : 0,
            has_hardfork( DPN_HARDFORK_0_20__1944 ) ? fc::ecc::bip_0062 : fc::ecc::fc_canonical );
      }
   ...
   if( BOOST_LIKELY(head_block_num() > 0) )
   {
      if( !(skip & skip_tapos_check) )
      {
         ...
         DPN_ASSERT( trx.ref_block_prefix == tapos_block_summary.block_id._hash[1], transaction_tapos_exception,
      ...
      DPN_ASSERT( trx.expiration <= now + fc::seconds(DPN_MAX_TIME_UNTIL_EXPIRATION), transaction_expiration_exception,
                  "", ("trx.expiration",trx.expiration)("now",now)("max_til_exp",DPN_MAX_TIME_UNTIL_EXPIRATION));
      if( has_hardfork( DPN_HARDFORK_0_9 ) ) // Simple solution to pending trx bug when now == trx.expiration
         DPN_ASSERT( now < trx.expiration, transaction_expiration_exception, "", ("now",now)("trx.exp",trx.expiration) );
      DPN_ASSERT( now <= trx.expiration, transaction_expiration_exception, "", ("now",now)("trx.exp",trx.expiration) );
   }
   ...
   while( true )
   {
      ...
      if( actions_itr == actions.end() )
      {
         ...
         if( pending_itr != pending_action_idx.end() && pending_itr->execution_time <= head_block_time() )
         {
            ...
            const auto& gpo = get_dynamic_global_properties();
            uint64_t required_actions_partition_size = ( gpo.maximum_block_size * gpo.required_actions_partition_percent ) / DPN_100_PERCENT;
...
{
   if( !has_hardfork( DPN_SMT_HARDFORK ) ) return;
...
FC_TODO( "#ifndef not needed after HF 20 is live" );
#ifndef IS_TEST_NET
               if( has_hardfork( DPN_HARDFORK_0_14__278 ) && !has_hardfork( DPN_HARDFORK_0_20__SP190 ) )
               {
                  if( head_block_num() - w.last_confirmed_block_num  > DPN_BLOCKS_PER_DAY )
                  {
   ...
   {
      DPN_ASSERT( _dgp.head_block_number - _dgp.last_irreversible_block_num  < DPN_MAX_UNDO_HISTORY, undo_database_exception,
                 "The database does not have enough undo history to support a blockchain with so many missed blocks. "
                 "Please add a checkpoint if you would like to continue applying blocks beyond this point.",
                 ("last_irreversible_block_num",_dgp.last_irreversible_block_num)("head", _dgp.head_block_number)
                 ("max_undo",DPN_MAX_UNDO_HISTORY) );
   }
} FC_CAPTURE_AND_RETHROW() }
...
{ try {
   modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& dgp )
   {
      dgp.virtual_supply = dgp.current_supply
         + ( get_feed_history().current_median_history.is_null() ? asset( 0, DPN_SYMBOL ) : dgp.current_dbd_supply * get_feed_history().current_median_history );

      auto median_price = get_feed_history().current_median_history;

      if( !median_price.is_null() && has_hardfork( DPN_HARDFORK_0_14__230 ) )
      {
         auto percent_dbd = uint16_t( ( ( fc::uint128_t( ( dgp.current_dbd_supply * get_feed_history().current_median_history ).amount.value ) * DPN_100_PERCENT )
            / dgp.virtual_supply.amount.value ).to_uint64() );

         if( percent_dbd <= dgp.dbd_start_percent )
            dgp.dbd_print_rate = DPN_100_PERCENT;
         else if( percent_dbd >= dgp.dbd_stop_percent - dgp.dbd_stop_adjust )
            dgp.dbd_print_rate = 0;
         else
            dgp.dbd_print_rate = ( ( dgp.dbd_stop_percent - percent_dbd ) * DPN_100_PERCENT ) / ( dgp.dbd_stop_percent - dgp.dbd_start_percent );
      }
   });
} FC_CAPTURE_AND_RETHROW() }
...
{ try {
    ...
    *
    */
   if( head_block_num() < DPN_START_MINER_VOTING_BLOCK )
   {
      modify( dpo, [&]( dynamic_global_property_object& _dpo )
      {
         if ( head_block_num() > DPN_MAX_WITNESSES )
            _dpo.last_irreversible_block_num = head_block_num() - DPN_MAX_WITNESSES;
      } );
   }
   else
   {
      ...
      static_assert( DPN_IRREVERSIBLE_THRESHOLD > 0, "irreversible threshold must be nonzero" );

      // 1 1 1 2 2 2 2 2 2 2 -> 2     .7*10 = 7
      // 1 1 1 1 1 1 1 2 2 2 -> 1
      // 3 3 3 3 3 3 3 3 3 3 -> 3

      size_t offset = ((DPN_100_PERCENT - DPN_IRREVERSIBLE_THRESHOLD) * wit_objs.size() / DPN_100_PERCENT);
...
{
   bool has_hf_20__1815 = has_hardfork( DPN_HARDFORK_0_20__1815 );

#pragma message( "TODO:  Remove if(), do assert unconditionally after HF20 occurs" )
   if( has_hf_20__1815 )
   {
      DPN_ASSERT( new_order.sell_price.quote.symbol == old_order.sell_price.base.symbol,
      ...
      DPN_ASSERT( new_order.sell_price.base.symbol  == old_order.sell_price.quote.symbol,
         order_match_exception, "error matching orders: ${new_order} ${old_order} ${match_price}",
         ("new_order", new_order)("old_order", old_order)("match_price", match_price) );
      DPN_ASSERT( new_order.for_sale > 0 && old_order.for_sale > 0,
         order_match_exception, "error matching orders: ${new_order} ${old_order} ${match_price}",
         ("new_order", new_order)("old_order", old_order)("match_price", match_price) );
      DPN_ASSERT( match_price.quote.symbol == new_order.sell_price.base.symbol,
         order_match_exception, "error matching orders: ${new_order} ${old_order} ${match_price}",
         ("new_order", new_order)("old_order", old_order)("match_price", match_price) );
      DPN_ASSERT( match_price.base.symbol == old_order.sell_price.base.symbol,
         order_match_exception, "error matching orders: ${new_order} ${old_order} ${match_price}",
         ("new_order", new_order)("old_order", old_order)("match_price", match_price) );
   }
...
#pragma message( "TODO:  Remove if(), do assert unconditionally after HF20 occurs" )
   if( has_hf_20__1815 )
   {
      DPN_ASSERT( new_order_pays == new_order.amount_for_sale() ||
   ...
   if( !has_hardfork( DPN_HARDFORK_0_12__178 ) &&
       ( (age >= DPN_MIN_LIQUIDITY_REWARD_PERIOD_SEC && !has_hardfork( DPN_HARDFORK_0_10__149)) ||
       (age >= DPN_MIN_LIQUIDITY_REWARD_PERIOD_SEC_HF10 && has_hardfork( DPN_HARDFORK_0_10__149) ) ) )
   {
      if( old_order_receives.symbol == DPN_SYMBOL )
      {
...
void database::adjust_liquidity_reward( const account_object& owner, const asset& volume, bool is_sdb )
{
   ...
   if( itr != ridx.end() )
   {
      ...
      {
         if( head_block_time() - r.last_update >= DPN_LIQUIDITY_TIMEOUT_SEC )
         {
            r.dbd_volume = 0;
            r.dpn_volume = 0;
            r.weight = 0;
         ...
         if( is_sdb )
            r.dbd_volume += volume.amount.value;
         else
            r.dpn_volume += volume.amount.value;

         r.update_weight( has_hardfork( DPN_HARDFORK_0_10__141 ) );
   ...
   else
   {
      create<liquidity_reward_balance_object>( [&](liquidity_reward_balance_object& r )
      {
         ...
         r.update_weight( has_hardfork( DPN_HARDFORK_0_9__141 ) );
         r.last_update = head_block_time();
      } );
   }
}
...
bool database::fill_order( const limit_order_object& order, const asset& pays, const asset& receives )
{
   try
   {
      ...
      {
#pragma message( "TODO:  Remove if(), do assert unconditionally after HF20 occurs" )
         if( has_hardfork( DPN_HARDFORK_0_20__1815 ) )
         {
            DPN_ASSERT( pays < order.amount_for_sale(),
              order_fill_exception, "error filling orders: ${order} ${pays} ${receives}",
              ("order", order)("pays", pays)("receives", receives) );
         }
...
void database::clear_expired_delegations()
{
   ...
   while( itr != delegations_by_exp.end() && itr->expiration < now )
   {
      ...
      {
         if( has_hardfork( DPN_HARDFORK_0_20__2539 ) )
         {
            util::update_manabar(
               gpo,
               a,
               has_hardfork( DPN_HARDFORK_0_21__3336 ),
   ...
   modify( a, [&]( account_object& acnt )
   {
      switch( delta.symbol.asset_num )
      {
         case DPN_ASSET_NUM_DPN:
            ...
            if( check_balance )
            {
               FC_ASSERT( acnt.balance.amount.value >= 0, "Insufficient DPN funds" );
            }
            break;
         case DPN_ASSET_NUM_DBD:
            if( a.dbd_seconds_last_update != head_block_time() )
            {
               acnt.dbd_seconds += fc::uint128_t(a.dbd_balance.amount.value) * (head_block_time() - a.dbd_seconds_last_update).to_seconds();
               acnt.dbd_seconds_last_update = head_block_time();

               if( acnt.dbd_seconds > 0 &&
                   (acnt.dbd_seconds_last_update - acnt.dbd_last_interest_payment).to_seconds() > DPN_DBD_INTEREST_COMPOUND_INTERVAL_SEC )
               {
                  auto interest = acnt.dbd_seconds / DPN_SECONDS_PER_YEAR;
                  interest *= get_dynamic_global_properties().dbd_interest_rate;
                  interest /= DPN_100_PERCENT;
                  asset interest_paid(interest.to_uint64(), DBD_SYMBOL);
                  acnt.dbd_balance += interest_paid;
                  acnt.dbd_seconds = 0;
                  acnt.dbd_last_interest_payment = head_block_time();
                  ...
                  {
                     props.current_dbd_supply += interest_paid;
                     props.virtual_supply += interest_paid * get_feed_history().current_median_history;
                  } );
               }
            }
            acnt.dbd_balance += delta;
            if( check_balance )
            {
               FC_ASSERT( acnt.dbd_balance.amount.value >= 0, "Insufficient DBD funds" );
            }
            break;
   ...
   modify( a, [&]( account_object& acnt )
   {
      switch( value_delta.symbol.asset_num )
      {
         case DPN_ASSET_NUM_DPN:
            ...
            {
               acnt.reward_dpn_balance += value_delta;
               if( check_balance )
               {
                  FC_ASSERT( acnt.reward_dpn_balance.amount.value >= 0, "Insufficient reward DPN funds" );
               }
            }
            else
            {
               acnt.reward_vesting_dpn += value_delta;
         ...
         case DPN_ASSET_NUM_DBD:
            ...
            acnt.reward_dbd_balance += value_delta;
            if( check_balance )
            {
               FC_ASSERT( acnt.reward_dbd_balance.amount.value >= 0, "Insufficient reward DBD funds" );
            }
            break;
         default:
            FC_ASSERT( false, "invalid symbol" );
...
{
   bool check_balance = has_hardfork( DPN_HARDFORK_0_20__1811 );
   ...
   modify( a, [&]( account_object& acnt )
   {
      switch( delta.symbol.asset_num )
      {
         case DPN_ASSET_NUM_DPN:
            ...
            if( check_balance )
            {
               FC_ASSERT( acnt.savings_balance.amount.value >= 0, "Insufficient savings DPN funds" );
            }
            break;
         case DPN_ASSET_NUM_DBD:
            if( a.savings_dbd_seconds_last_update != head_block_time() )
            {
               acnt.savings_dbd_seconds += fc::uint128_t(a.savings_dbd_balance.amount.value) * (head_block_time() - a.savings_dbd_seconds_last_update).to_seconds();
               acnt.savings_dbd_seconds_last_update = head_block_time();

               if( acnt.savings_dbd_seconds > 0 &&
                   (acnt.savings_dbd_seconds_last_update - acnt.savings_dbd_last_interest_payment).to_seconds() > DPN_DBD_INTEREST_COMPOUND_INTERVAL_SEC )
               {
                  auto interest = acnt.savings_dbd_seconds / DPN_SECONDS_PER_YEAR;
                  interest *= get_dynamic_global_properties().dbd_interest_rate;
                  interest /= DPN_100_PERCENT;
                  asset interest_paid(interest.to_uint64(), DBD_SYMBOL);
                  acnt.savings_dbd_balance += interest_paid;
                  acnt.savings_dbd_seconds = 0;
                  acnt.savings_dbd_last_interest_payment = head_block_time();
            ...
            acnt.savings_dbd_balance += delta;
            if( check_balance )
            {
               FC_ASSERT( acnt.savings_dbd_balance.amount.value >= 0, "Insufficient savings DBD funds" );
            }
            break;
         default:
...
void database::adjust_supply( const asset& delta, bool adjust_vesting )
{
   ...
   bool check_supply = has_hardfork( DPN_HARDFORK_0_20__1811 );

   const auto& props = get_dynamic_global_properties();
   if( props.head_block_number < DPN_BLOCKS_PER_DAY*7 )
   ...
   modify( props, [&]( dynamic_global_property_object& props )
   {
      switch( delta.symbol.asset_num )
      {
         case DPN_ASSET_NUM_DPN:
         {
            asset new_vesting( (adjust_vesting && delta.amount > 0) ? delta.amount * 9 : 0, DPN_SYMBOL );
            props.current_supply += delta + new_vesting;
            props.virtual_supply += delta + new_vesting;
            props.total_vesting_fund_dpn += new_vesting;
            if( check_supply )
            {
         ...
         case DPN_ASSET_NUM_DBD:
            props.current_dbd_supply += delta;
            props.virtual_supply = props.current_dbd_supply * get_feed_history().current_median_history + props.current_supply;
            if( check_supply )
            {
               FC_ASSERT( props.current_dbd_supply.amount.value >= 0 );
            }
            break;
         default:
            FC_ASSERT( false, "invalid symbol" );
...
void database::generate_optional_actions()
{
   ...
   DPN_TRY_NOTIFY( _generate_optional_actions_signal, note );
}

void database::init_hardforks()
{
   _hardfork_versions.times[ 0 ] = fc::time_point_sec( DPN_GENESIS_TIME );
   _hardfork_versions.versions[ 0 ] = hardfork_version( 0, 0 );
   FC_ASSERT( DPN_HARDFORK_0_1 == 1, "Invalid hardfork configuration" );
   _hardfork_versions.times[ DPN_HARDFORK_0_1 ] = fc::time_point_sec( DPN_HARDFORK_0_1_TIME );
   _hardfork_versions.versions[ DPN_HARDFORK_0_1 ] = DPN_HARDFORK_0_1_VERSION;
   FC_ASSERT( DPN_HARDFORK_0_2 == 2, "Invlaid hardfork configuration" );
   _hardfork_versions.times[ DPN_HARDFORK_0_2 ] = fc::time_point_sec( DPN_HARDFORK_0_2_TIME );
   _hardfork_versions.versions[ DPN_HARDFORK_0_2 ] = DPN_HARDFORK_0_2_VERSION;
   FC_ASSERT( DPN_HARDFORK_0_3 == 3, "Invalid hardfork configuration" );
   _hardfork_versions.times[ DPN_HARDFORK_0_3 ] = fc::time_point_sec( DPN_HARDFORK_0_3_TIME );
   _hardfork_versions.versions[ DPN_HARDFORK_0_3 ] = DPN_HARDFORK_0_3_VERSION;
   FC_ASSERT( DPN_HARDFORK_0_4 == 4, "Invalid hardfork configuration" );
   _hardfork_versions.times[ DPN_HARDFORK_0_4 ] = fc::time_point_sec( DPN_HARDFORK_0_4_TIME );
   _hardfork_versions.versions[ DPN_HARDFORK_0_4 ] = DPN_HARDFORK_0_4_VERSION;
   FC_ASSERT( DPN_HARDFORK_0_5 == 5, "Invalid hardfork configuration" );
   _hardfork_versions.times[ DPN_HARDFORK_0_5 ] = fc::time_point_sec( DPN_HARDFORK_0_5_TIME );
   _hardfork_versions.versions[ DPN_HARDFORK_0_5 ] = DPN_HARDFORK_0_5_VERSION;
   FC_ASSERT( DPN_HARDFORK_0_6 == 6, "Invalid hardfork configuration" );
   _hardfork_versions.times[ DPN_HARDFORK_0_6 ] = fc::time_point_sec( DPN_HARDFORK_0_6_TIME );
   _hardfork_versions.versions[ DPN_HARDFORK_0_6 ] = DPN_HARDFORK_0_6_VERSION;
   FC_ASSERT( DPN_HARDFORK_0_7 == 7, "Invalid hardfork configuration" );
   _hardfork_versions.times[ DPN_HARDFORK_0_7 ] = fc::time_point_sec( DPN_HARDFORK_0_7_TIME );
   _hardfork_versions.versions[ DPN_HARDFORK_0_7 ] = DPN_HARDFORK_0_7_VERSION;
   FC_ASSERT( DPN_HARDFORK_0_8 == 8, "Invalid hardfork configuration" );
   _hardfork_versions.times[ DPN_HARDFORK_0_8 ] = fc::time_point_sec( DPN_HARDFORK_0_8_TIME );
   _hardfork_versions.versions[ DPN_HARDFORK_0_8 ] = DPN_HARDFORK_0_8_VERSION;
   FC_ASSERT( DPN_HARDFORK_0_9 == 9, "Invalid hardfork configuration" );
   _hardfork_versions.times[ DPN_HARDFORK_0_9 ] = fc::time_point_sec( DPN_HARDFORK_0_9_TIME );
   _hardfork_versions.versions[ DPN_HARDFORK_0_9 ] = DPN_HARDFORK_0_9_VERSION;
   FC_ASSERT( DPN_HARDFORK_0_10 == 10, "Invalid hardfork configuration" );
   _hardfork_versions.times[ DPN_HARDFORK_0_10 ] = fc::time_point_sec( DPN_HARDFORK_0_10_TIME );
   _hardfork_versions.versions[ DPN_HARDFORK_0_10 ] = DPN_HARDFORK_0_10_VERSION;
   FC_ASSERT( DPN_HARDFORK_0_11 == 11, "Invalid hardfork configuration" );
   _hardfork_versions.times[ DPN_HARDFORK_0_11 ] = fc::time_point_sec( DPN_HARDFORK_0_11_TIME );
   _hardfork_versions.versions[ DPN_HARDFORK_0_11 ] = DPN_HARDFORK_0_11_VERSION;
   FC_ASSERT( DPN_HARDFORK_0_12 == 12, "Invalid hardfork configuration" );
   _hardfork_versions.times[ DPN_HARDFORK_0_12 ] = fc::time_point_sec( DPN_HARDFORK_0_12_TIME );
   _hardfork_versions.versions[ DPN_HARDFORK_0_12 ] = DPN_HARDFORK_0_12_VERSION;
   FC_ASSERT( DPN_HARDFORK_0_13 == 13, "Invalid hardfork configuration" );
   _hardfork_versions.times[ DPN_HARDFORK_0_13 ] = fc::time_point_sec( DPN_HARDFORK_0_13_TIME );
   _hardfork_versions.versions[ DPN_HARDFORK_0_13 ] = DPN_HARDFORK_0_13_VERSION;
   FC_ASSERT( DPN_HARDFORK_0_14 == 14, "Invalid hardfork configuration" );
   _hardfork_versions.times[ DPN_HARDFORK_0_14 ] = fc::time_point_sec( DPN_HARDFORK_0_14_TIME );
   _hardfork_versions.versions[ DPN_HARDFORK_0_14 ] = DPN_HARDFORK_0_14_VERSION;
   FC_ASSERT( DPN_HARDFORK_0_15 == 15, "Invalid hardfork configuration" );
   _hardfork_versions.times[ DPN_HARDFORK_0_15 ] = fc::time_point_sec( DPN_HARDFORK_0_15_TIME );
   _hardfork_versions.versions[ DPN_HARDFORK_0_15 ] = DPN_HARDFORK_0_15_VERSION;
   FC_ASSERT( DPN_HARDFORK_0_16 == 16, "Invalid hardfork configuration" );
   _hardfork_versions.times[ DPN_HARDFORK_0_16 ] = fc::time_point_sec( DPN_HARDFORK_0_16_TIME );
   _hardfork_versions.versions[ DPN_HARDFORK_0_16 ] = DPN_HARDFORK_0_16_VERSION;
   FC_ASSERT( DPN_HARDFORK_0_17 == 17, "Invalid hardfork configuration" );
   _hardfork_versions.times[ DPN_HARDFORK_0_17 ] = fc::time_point_sec( DPN_HARDFORK_0_17_TIME );
   _hardfork_versions.versions[ DPN_HARDFORK_0_17 ] = DPN_HARDFORK_0_17_VERSION;
   FC_ASSERT( DPN_HARDFORK_0_18 == 18, "Invalid hardfork configuration" );
   _hardfork_versions.times[ DPN_HARDFORK_0_18 ] = fc::time_point_sec( DPN_HARDFORK_0_18_TIME );
   _hardfork_versions.versions[ DPN_HARDFORK_0_18 ] = DPN_HARDFORK_0_18_VERSION;
   FC_ASSERT( DPN_HARDFORK_0_19 == 19, "Invalid hardfork configuration" );
   _hardfork_versions.times[ DPN_HARDFORK_0_19 ] = fc::time_point_sec( DPN_HARDFORK_0_19_TIME );
   _hardfork_versions.versions[ DPN_HARDFORK_0_19 ] = DPN_HARDFORK_0_19_VERSION;
   FC_ASSERT( DPN_HARDFORK_0_20 == 20, "Invalid hardfork configuration" );
   _hardfork_versions.times[ DPN_HARDFORK_0_20 ] = fc::time_point_sec( DPN_HARDFORK_0_20_TIME );
   _hardfork_versions.versions[ DPN_HARDFORK_0_20 ] = DPN_HARDFORK_0_20_VERSION;
   FC_ASSERT( DPN_HARDFORK_0_21 == 21, "Invalid hardfork configuration" );
   _hardfork_versions.times[ DPN_HARDFORK_0_21 ] = fc::time_point_sec( DPN_HARDFORK_0_21_TIME );
   _hardfork_versions.versions[ DPN_HARDFORK_0_21 ] = DPN_HARDFORK_0_21_VERSION;
#ifdef IS_TEST_NET
   FC_ASSERT( DPN_HARDFORK_0_22 == 22, "Invalid hardfork configuration" );
   _hardfork_versions.times[ DPN_HARDFORK_0_22 ] = fc::time_point_sec( DPN_HARDFORK_0_22_TIME );
   _hardfork_versions.versions[ DPN_HARDFORK_0_22 ] = DPN_HARDFORK_0_22_VERSION;
#endif


   const auto& hardforks = get_hardfork_property_object();
   FC_ASSERT( hardforks.last_hardfork <= DPN_NUM_HARDFORKS, "Chain knows of more hardforks than configuration", ("hardforks.last_hardfork",hardforks.last_hardfork)("DPN_NUM_HARDFORKS",DPN_NUM_HARDFORKS) );
   FC_ASSERT( _hardfork_versions.versions[ hardforks.last_hardfork ] <= DPN_BLOCKCHAIN_VERSION, "Blockchain version is older than last applied hardfork" );
   FC_ASSERT( DPN_BLOCKCHAIN_HARDFORK_VERSION >= DPN_BLOCKCHAIN_VERSION );
   FC_ASSERT( DPN_BLOCKCHAIN_HARDFORK_VERSION == _hardfork_versions.versions[ DPN_NUM_HARDFORKS ] );
}
...
void database::process_hardforks()
{
   try
   {
      ...
      {
         while( hardforks.last_hardfork < DPN_NUM_HARDFORKS
               && _hardfork_versions.times[ hardforks.last_hardfork + 1 ] <= head_block_time()
               && hardforks.last_hardfork < DPN_HARDFORK_0_5__54 )
         {
            apply_hardfork( hardforks.last_hardfork + 1 );
         }
      }
...
void database::set_hardfork( uint32_t hardfork, bool apply_now )
{
   ...
   for( uint32_t i = hardforks.last_hardfork + 1; i <= hardfork && i <= DPN_NUM_HARDFORKS; i++ )
   {
      if( i <= DPN_HARDFORK_0_5__54 )
...
void database::apply_hardfork( uint32_t hardfork )
{
   ...
   switch( hardfork )
   {
      case DPN_HARDFORK_0_1:
         perform_vesting_share_split( 1000000 );
         break;
      case DPN_HARDFORK_0_2:
         retally_witness_votes();
         break;
      case DPN_HARDFORK_0_3:
         retally_witness_votes();
         break;
      case DPN_HARDFORK_0_4:
         reset_virtual_schedule_time(*this);
         break;
      case DPN_HARDFORK_0_5:
         break;
      case DPN_HARDFORK_0_6:
         retally_witness_vote_counts();
         retally_comment_children();
         break;
      case DPN_HARDFORK_0_7:
         break;
      case DPN_HARDFORK_0_8:
         retally_witness_vote_counts(true);
         break;
      case DPN_HARDFORK_0_9:
         {
         ...
         break;
      case DPN_HARDFORK_0_10:
         retally_liquidity_weight();
         break;
      case DPN_HARDFORK_0_11:
         break;
      case DPN_HARDFORK_0_12:
         {
            ...
            for( auto itr = comment_idx.begin(); itr != comment_idx.end(); ++itr )
            {
               ...
               if( itr->parent_author == DPN_ROOT_POST_PARENT )
               {
                  ...
                  if( itr->last_payout == fc::time_point_sec::min() && itr->cashout_time == fc::time_point_sec::maximum() )
                  {
                     ...
                     {
                        c.cashout_time = head_block_time() + DPN_CASHOUT_WINDOW_SECONDS_PRE_HF17;
                     });
                  }
                  ...
                  else if( itr->last_payout > fc::time_point_sec() )
                  {
                     modify( *itr, [&]( comment_object& c )
                     {
                        c.cashout_time = c.last_payout + DPN_SECOND_CASHOUT_WINDOW;
                     });
                  }
               }
            }

            modify( get< account_authority_object, by_account >( DPN_MINER_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });

            modify( get< account_authority_object, by_account >( DPN_NULL_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });

            modify( get< account_authority_object, by_account >( DPN_TEMP_ACCOUNT ), [&]( account_authority_object& auth )
            {
               auth.posting = authority();
               auth.posting.weight_threshold = 1;
            });
         ...
         break;
      case DPN_HARDFORK_0_13:
         break;
      case DPN_HARDFORK_0_14:
         break;
      case DPN_HARDFORK_0_15:
         break;
      case DPN_HARDFORK_0_16:
         {
            modify( get_feed_history(), [&]( feed_history_object& fho )
            {
               while( fho.price_history.size() > DPN_FEED_HISTORY_WINDOW )
                  fho.price_history.pop_front();
            });
         }
         break;
      case DPN_HARDFORK_0_17:
         {
            static_assert(
               DPN_MAX_VOTED_WITNESSES_HF0 + DPN_MAX_MINER_WITNESSES_HF0 + DPN_MAX_RUNNER_WITNESSES_HF0 == DPN_MAX_WITNESSES,
               "HF0 witness counts must add up to DPN_MAX_WITNESSES" );
            static_assert(
            ...
            {
               rfo.name = DPN_POST_REWARD_FUND_NAME;
               rfo.last_update = head_block_time();
               rfo.content_constant = DPN_CONTENT_CONSTANT_HF0;
               rfo.percent_curation_rewards = DPN_1_PERCENT * 25;
               rfo.percent_content_rewards = DPN_100_PERCENT;
               rfo.reward_balance = gpo.total_reward_fund_dpn;
#ifndef IS_TEST_NET
               rfo.recent_claims = DPN_HF_17_RECENT_CLAIMS;
#endif
            ...
            modify( gpo, [&]( dynamic_global_property_object& g )
            {
               g.total_reward_fund_dpn = asset( 0, DPN_SYMBOL );
            ...
            */
            ...
            root_posts.reserve( DPN_HF_17_NUM_POSTS );
            vector< const comment_object* > replies;
            replies.reserve( DPN_HF_17_NUM_REPLIES );
            ...
            for( const auto& itr : root_posts )
            {
               modify( *itr, [&]( comment_object& c )
               {
                  c.cashout_time = std::max( c.created + DPN_CASHOUT_WINDOW_SECONDS, c.cashout_time );
               });
            }
            ...
            for( const auto& itr : replies )
            {
               modify( *itr, [&]( comment_object& c )
               {
                  c.cashout_time = std::max( calculate_discussion_payout_time( c ), c.created + DPN_CASHOUT_WINDOW_SECONDS );
               });
            }
         }
         break;
      case DPN_HARDFORK_0_18:
         break;
      case DPN_HARDFORK_0_19:
         {
            modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
            {
               gpo.vote_power_reserve_rate = DPN_REDUCED_VOTE_POWER_RATE;
            });

            modify( get< reward_fund_object, by_name >( DPN_POST_REWARD_FUND_NAME ), [&]( reward_fund_object &rfo )
            {
#ifndef IS_TEST_NET
               rfo.recent_claims = DPN_HF_19_RECENT_CLAIMS;
#endif
         ...
         break;
      case DPN_HARDFORK_0_20:
         {
            modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
            {
               gpo.delegation_return_period = DPN_DELEGATION_RETURN_PERIOD_HF20;
               gpo.reverse_auction_seconds = DPN_REVERSE_AUCTION_WINDOW_SECONDS_HF20;
               gpo.dbd_stop_percent = DPN_DBD_STOP_PERCENT_HF20;
               gpo.dbd_start_percent = DPN_DBD_START_PERCENT_HF20;
            ...
            for( const auto& witness : wso.current_shuffled_witnesses )
            {
               ...
               if( witness != account_name_type() )
               {
                  ...
                  {
                     w.props.account_creation_fee = asset( w.props.account_creation_fee.amount * DPN_CREATE_ACCOUNT_WITH_DPN_MODIFIER, DPN_SYMBOL );
                  });
               }
            }
            ...
            {
               wso.median_props.account_creation_fee = asset( wso.median_props.account_creation_fee.amount * DPN_CREATE_ACCOUNT_WITH_DPN_MODIFIER, DPN_SYMBOL );
            });
         }
         break;
      case DPN_HARDFORK_0_21:
      {
         modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
         {
            gpo.dbd_stop_adjust = DPN_DBD_STOP_ADJUST;
            gpo.sps_fund_percent = DPN_PROPOSAL_FUND_PERCENT_HF21;
            gpo.content_reward_percent = DPN_CONTENT_REWARD_PERCENT_HF21;
            gpo.downvote_pool_percent = DPN_DOWNVOTE_POOL_PERCENT_HF21;
            gpo.reverse_auction_seconds = DPN_REVERSE_AUCTION_WINDOW_SECONDS_HF21;
         });

         auto account_auth = find< account_authority_object, by_account >( DPN_TREASURY_ACCOUNT );
         if( account_auth == nullptr )
            create< account_authority_object >( [&]( account_authority_object& auth )
            {
               auth.account = DPN_TREASURY_ACCOUNT;
               auth.owner.weight_threshold = 1;
               auth.active.weight_threshold = 1;
               auth.posting.weight_threshold = 1;
            });
         ...
         modify( get_account( DPN_TREASURY_ACCOUNT ), [&]( account_object& a )
         {
            a.recovery_account = DPN_TREASURY_ACCOUNT;
         });

         auto rec_req = find< account_recovery_request_object, by_account >( DPN_TREASURY_ACCOUNT );
         if( rec_req )
            remove( *rec_req );

         auto change_request = find< change_recovery_account_request_object, by_account >( DPN_TREASURY_ACCOUNT );
         if( change_request )
            remove( *change_request );

         modify( get< reward_fund_object, by_name >( DPN_POST_REWARD_FUND_NAME ), [&]( reward_fund_object& rfo )
         {
            rfo.percent_curation_rewards = 50 * DPN_1_PERCENT;
            rfo.author_reward_curve = convergent_linear;
            rfo.curation_reward_curve = convergent_square_root;
            rfo.content_constant = DPN_CONTENT_CONSTANT_HF21;
#ifndef  IS_TEST_NET
            rfo.recent_claims = DPN_HF21_CONVERGENT_LINEAR_RECENT_CLAIMS;
#endif
         });
      }
      break;
      case DPN_SMT_HARDFORK:
      {
#ifdef DPN_ENABLE_SMT
         ...
         modify( get_dynamic_global_properties(), [&]( dynamic_global_property_object& gpo )
         {
            gpo.required_actions_partition_percent = 25 * DPN_1_PERCENT;
         });
...
void database::validate_invariants()const
{
   try
   {
      ...
      asset total_supply = asset( 0, DPN_SYMBOL );
      asset total_dbd = asset( 0, DBD_SYMBOL );
      asset total_vesting = asset( 0, VESTS_SYMBOL );
      asset pending_vesting_dpn = asset( 0, DPN_SYMBOL );
      ...
      for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
      {
         ...
         total_supply += itr->reward_dpn_balance;
         total_dbd += itr->dbd_balance;
         total_dbd += itr->savings_dbd_balance;
         total_dbd += itr->reward_dbd_balance;
         total_vesting += itr->vesting_shares;
         total_vesting += itr->reward_vesting_balance;
         pending_vesting_dpn += itr->reward_vesting_dpn;
         total_vsf_votes += ( itr->proxy == DPN_PROXY_TO_SELF_ACCOUNT ?
                                 itr->witness_vote_weight() :
                                 ( DPN_MAX_PROXY_RECURSION_DEPTH > 0 ?
                                      itr->proxied_vsf_votes[DPN_MAX_PROXY_RECURSION_DEPTH - 1] :
      ...
      {
         if( itr->amount.symbol == DPN_SYMBOL )
            total_supply += itr->amount;
         else if( itr->amount.symbol == DBD_SYMBOL )
            total_dbd += itr->amount;
         else
      ...
      for( auto itr = limit_order_idx.begin(); itr != limit_order_idx.end(); ++itr )
      {
         ...
         {
            total_supply += asset( itr->for_sale, DPN_SYMBOL );
         }
         else if ( itr->sell_price.base.symbol == DBD_SYMBOL )
         {
            total_dbd += asset( itr->for_sale, DBD_SYMBOL );
         }
      }
      ...
      {
         total_supply += itr->dpn_balance;
         total_dbd += itr->dbd_balance;

         if( itr->pending_fee.symbol == DPN_SYMBOL )
            total_supply += itr->pending_fee;
         else if( itr->pending_fee.symbol == DBD_SYMBOL )
            total_dbd += itr->pending_fee;
         else
            FC_ASSERT( false, "found escrow pending fee that is not DBD or DPN" );
      }
      ...
      for( auto itr = savings_withdraw_idx.begin(); itr != savings_withdraw_idx.end(); ++itr )
      {
         if( itr->amount.symbol == DPN_SYMBOL )
            ...
            FC_ASSERT( false, "found savings withdraw that is not DBD or DPN" );
      }
      ...
      total_supply += gpo.total_vesting_fund_dpn + gpo.total_reward_fund_dpn + gpo.pending_rewarded_vesting_dpn;

      FC_ASSERT( gpo.current_supply == total_supply, "", ("gpo.current_supply",gpo.current_supply)("total_supply",total_supply) );
      FC_ASSERT( gpo.current_dbd_supply + gpo.init_dbd_supply == total_dbd, "", ("gpo.current_dbd_supply",gpo.current_dbd_supply)("total_dbd",total_dbd) );
      ...
      FC_ASSERT( gpo.pending_rewarded_vesting_dpn == pending_vesting_dpn, "", ("pending_rewarded_vesting_dpn",gpo.pending_rewarded_vesting_dpn)("pending_vesting_dpn", pending_vesting_dpn));

      FC_ASSERT( gpo.virtual_supply >= gpo.current_supply );
      if ( !get_feed_history().current_median_history.is_null() )
      {
         FC_ASSERT( gpo.current_dbd_supply * get_feed_history().current_median_history + gpo.current_supply
            == gpo.virtual_supply, "", ("gpo.current_dbd_supply",gpo.current_dbd_supply)("get_feed_history().current_median_history",get_feed_history().current_median_history)("gpo.current_supply",gpo.current_supply)("gpo.virtual_supply",gpo.virtual_supply) );
      }
   }
...
void database::validate_smt_invariants()const
{
   try
   {
      ...
      // - Process reward balances, collecting SMT counterparts of 'reward_dpn_balance', 'reward_vesting_balance' & 'reward_vesting_dpn'.
   ...
   try
   {
      ...
      for( const auto& account : get_index< account_index, by_id >() )
      {
         modify( account, [&]( account_object& a )
         {
            ...
            a.vesting_withdraw_rate  = asset( a.to_withdraw / DPN_VESTING_WITHDRAW_INTERVALS_PRE_HF_16, VESTS_SYMBOL );
            if( a.vesting_withdraw_rate.amount == 0 )
               a.vesting_withdraw_rate.amount = 1;

            for( uint32_t i = 0; i < DPN_MAX_PROXY_RECURSION_DEPTH; ++i )
...
// Low memory nodes only need immediate child count, full nodes track total children
         ...
         while( parent )
         {
            ...
            if( parent->parent_author != DPN_ROOT_POST_PARENT )
...
void database::retally_witness_votes()
{
   ...
   for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
   {
      if( itr->proxy != DPN_PROXY_TO_SELF_ACCOUNT ) continue;
...
void database::retally_witness_vote_counts( bool force )
{
   ...
   for( auto itr = account_idx.begin(); itr != account_idx.end(); ++itr )
   {
      ...
      if( force || (a.proxy != DPN_PROXY_TO_SELF_ACCOUNT  ) )
      {
        const auto& vidx = get_index< witness_vote_index >().indices().get< by_account_witness >();
        auto wit_itr = vidx.lower_bound( boost::make_tuple( a.name, account_name_type() ) );
        while( wit_itr != vidx.end() && wit_itr->account == a.name )
