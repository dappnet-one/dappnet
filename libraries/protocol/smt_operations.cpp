...
void smt_create_operation::validate()const
{
   ...
   FC_ASSERT( smt_creation_fee.amount <= DPN_MAX_SHARE_SUPPLY, "Fee must be smaller than DPN_MAX_SHARE_SUPPLY" );
   FC_ASSERT( is_asset_type( smt_creation_fee, DPN_SYMBOL ) || is_asset_type( smt_creation_fee, DBD_SYMBOL ), "Fee must be DPN or DBD" );
...
uint32_t smt_generation_unit::dpn_unit_sum()const
{
   uint32_t result = 0;
   for(const std::pair< account_name_type, uint16_t >& e : dpn_unit )
      result += e.second;
   return result;
}
...
void smt_capped_generation_policy::validate()const
{
   ...
   FC_ASSERT( soft_cap_percent <= DPN_100_PERCENT );

   FC_ASSERT( pre_soft_cap_unit.dpn_unit.size() > 0 );
   FC_ASSERT( pre_soft_cap_unit.token_unit.size() > 0 );

   FC_ASSERT( pre_soft_cap_unit.dpn_unit.size() <= SMT_MAX_UNIT_COUNT );
   FC_ASSERT( pre_soft_cap_unit.token_unit.size() <= SMT_MAX_UNIT_COUNT );
   FC_ASSERT( post_soft_cap_unit.dpn_unit.size() <= SMT_MAX_UNIT_COUNT );
   FC_ASSERT( post_soft_cap_unit.token_unit.size() <= SMT_MAX_UNIT_COUNT );

   // TODO : Check account name

   if( soft_cap_percent == DPN_100_PERCENT )
   {
      FC_ASSERT( post_soft_cap_unit.dpn_unit.size() == 0 );
...
void smt_setup_emissions_operation::validate()const
{
   ...
   FC_ASSERT( schedule_time > DPN_GENESIS_TIME );
...
void smt_setup_operation::validate()const
{
   ...
   FC_ASSERT( max_supply <= DPN_MAX_SHARE_SUPPLY, "Max supply must be less than ${n}", ("n", DPN_MAX_SHARE_SUPPLY) );
   FC_ASSERT( contribution_begin_time > DPN_GENESIS_TIME, "Contribution begin time must be greater than ${t}", ("t", DPN_GENESIS_TIME) );
   ...
   FC_ASSERT( dpn_units_soft_cap <= dpn_units_hard_cap, "Dpn units soft cap must less than or equal to dpn units hard cap" );
   FC_ASSERT( dpn_units_soft_cap >= SMT_MIN_SOFT_CAP_DPN_UNITS, "Dpn units soft cap must be greater than or equal to ${n}", ("n", SMT_MIN_SOFT_CAP_DPN_UNITS) );
   FC_ASSERT( dpn_units_hard_cap >= SMT_MIN_HARD_CAP_DPN_UNITS, "Dpn units hard cap must be greater than or equal to ${n}", ("n", SMT_MIN_HARD_CAP_DPN_UNITS) );
   FC_ASSERT( dpn_units_hard_cap <= DPN_MAX_SHARE_SUPPLY, "Dpn units hard cap must be less than or equal to ${n}", ("n", DPN_MAX_SHARE_SUPPLY) );
...
struct smt_set_runtime_parameters_operation_visitor
{
   ...
   {
      FC_ASSERT( param_rewards.percent_curation_rewards <= DPN_100_PERCENT,
...
void smt_contribute_operation::validate() const
{
   ...
   FC_ASSERT( contribution.symbol == DPN_SYMBOL, "Contributions must be made in DPN" );
