...
#define SMT_MIN_HARD_CAP_DPN_UNITS   10000
#define SMT_MIN_SATURATION_DPN_UNITS 1000
#define SMT_MIN_SOFT_CAP_DPN_UNITS   1000
...
{
   flat_map< account_name_type, uint16_t >        dpn_unit;
   flat_map< account_name_type, uint16_t >        token_unit;

   uint32_t dpn_unit_sum()const;
...
struct smt_setup_operation : public base_operation
{
   ...
   int64_t                 max_supply = DPN_MAX_SHARE_SUPPLY;
   ...
   share_type              dpn_units_soft_cap;
   share_type              dpn_units_hard_cap;
...
{
   uint32_t cashout_window_seconds = 0;                // DPN_CASHOUT_WINDOW_SECONDS
   uint32_t reverse_auction_window_seconds = 0;        // DPN_REVERSE_AUCTION_WINDOW_SECONDS
};

struct smt_param_vote_regeneration_period_seconds_v1
{
   uint32_t vote_regeneration_period_seconds = 0;      // DPN_VOTING_MANA_REGENERATION_SECONDS
...
FC_REFLECT(
   dpn::protocol::smt_create_operation,
   (control_account)
   (symbol)
   (smt_creation_fee)
   (extensions)
...
FC_REFLECT(
   dpn::protocol::smt_setup_operation,
   (control_account)
   (symbol)
   (max_supply)
...
FC_REFLECT(
   dpn::protocol::smt_generation_unit,
   (dpn_unit)
   (token_unit)
   )
...
FC_REFLECT(
   dpn::protocol::smt_capped_generation_policy,
   (pre_soft_cap_unit)
   (post_soft_cap_unit)
   (soft_cap_percent)
   (min_unit_ratio)
...
FC_REFLECT(
   dpn::protocol::smt_emissions_unit,
   (token_unit)
   )

FC_REFLECT(
   dpn::protocol::smt_setup_emissions_operation,
   (control_account)
   (symbol)
   (schedule_time)
   (emissions_unit)
...
FC_REFLECT(
   dpn::protocol::smt_param_allow_voting,
   (value)
   )

FC_REFLECT_TYPENAME( dpn::protocol::smt_setup_parameter )

FC_REFLECT(
   dpn::protocol::smt_param_windows_v1,
   (cashout_window_seconds)
   (reverse_auction_window_seconds)
   )

FC_REFLECT(
   dpn::protocol::smt_param_vote_regeneration_period_seconds_v1,
   (vote_regeneration_period_seconds)
   (votes_per_regeneration_period)
   )

FC_REFLECT(
   dpn::protocol::smt_param_rewards_v1,
   (content_constant)
...
FC_REFLECT(
   dpn::protocol::smt_param_allow_downvotes,
   (value)
)

FC_REFLECT_TYPENAME(
   dpn::protocol::smt_runtime_parameter
   )

FC_REFLECT(
   dpn::protocol::smt_set_setup_parameters_operation,
   (control_account)
   (symbol)
   (setup_parameters)
   (extensions)
...
FC_REFLECT(
   dpn::protocol::smt_set_runtime_parameters_operation,
   (control_account)
   (symbol)
   (runtime_parameters)
   (extensions)
...
FC_REFLECT(
   dpn::protocol::smt_contribute_operation,
   (contributor)
   (symbol)
   (contribution_id)
   (contribution)
